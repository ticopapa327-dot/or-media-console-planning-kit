...
| 医院接口服务 | HIS/EMR/PACS/IAM/存储接口适配 | 标准优先，接口隔离，重试和熔断 |
...
| 接口 | 首选标准 | 替代方案 | 风险 |
| --- | --- | --- | --- |
| 患者和排班 | HL7 v2、FHIR | 数据库视图、WebService、厂商 API | 字段差异、接口稳定性 |
...
| `patients` | id, mrn_hash, name_token, sex, birth_date, source_system |
